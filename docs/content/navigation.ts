...
const pageModules: Record<string, () => Promise<{ default: { title: string; content: string } }>> = {
  ...
  logging: () => import('./pages/logging.ts'),
  mail: () => import('./pages/mail.ts'),
  notifications: () => import('./pages/notifications.ts'),
  queues: () => import('./pages/queues.ts'),
  realtime: () => import('./pages/realtime.ts'),
  helpers: () => import('./pages/helpers.ts'),
