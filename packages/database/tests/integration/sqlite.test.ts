/**
 * Integration tests against a real SQLite database (in-memory via bun:sqlite).
 * Tests migrations, DDL for all column types, query builder CRUD, ORM, factories,
 * transactions, and schema operations.
 *
 * Run: bun test packages/database/tests/integration/sqlite.test.ts
 */
...
// Add relations to User after all classes are defined
...
describe('SQLite Migration DDL — all column types', () => {
  ...
  class CreateForeignKeyTable extends Migration {
    async up(schema: SchemaBuilder) {
      await schema.create('ddl_posts', (t) => {
        ...
        t.timestamps()
        // Note: SQLite FKs are defined inline in the column but we track them
        // via PRAGMA foreign_keys. The foreign() call stores info for non-SQLite
        // drivers; SQLite handles constraints through PRAGMA.
      })
    }
    async down(schema: SchemaBuilder) {
  ...
  test('enum column accepts valid values (TEXT in SQLite)', async () => {
    const migrator = new Migrator(conn)
    await migrator.run([
      { name: '001_create_all_columns', migration: new CreateAllColumnsTable() },
...
describe('SQLite Schema & QueryBuilder', () => {
  ...
  beforeEach(async () => {
    ...
    await conn.table('user_tags').delete()
  ...
  test('whereRaw', async () => {
    ...
    const rows = await conn.table('users').whereRaw('LENGTH(name) > ?', [6]).get()
    expect(rows).toHaveLength(1)
  ...
  test('paginate returns correct metadata', async () => {
    ...
    const result = await conn.table('users').paginate(2, 2)
